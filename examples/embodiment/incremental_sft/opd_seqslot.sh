#!/bin/bash
# opd_seqslot.sh -- seqslot: SEQUENTIAL per-suite rounds over the slot-LoRI student, in ONE
# 24-step process. Derived from opd_mt4slot.sh; same guard rails (disk floor, GPU preflight,
# checkpoint reaper, run_iso isolation), three deliberate differences:
#
#   * config = libero_seqslot_4gpu.yaml -- round_table, anchor_lambda 0.3 / anchor_mode
#     self_masked, alt_schedule "B", frozen_orth. Every mechanism is pinned THERE, next to the
#     comment explaining it; this script adds no hydra overrides on top.
#   * the reaper WHITELISTS the round-end checkpoints (steps 6,12,18,24): they are M_1..M_4,
#     the per-round artefacts the analysis needs, and the only ones besides `best` and the
#     newest two that survive the run.
#   * default GPUS=0,1,2,3 and STEPS=24 (4 rounds x 6 steps). Round boundaries live in the
#     config's round_table -- if STEPS changes, change the table WITH it or the later rounds
#     simply never run (the lookup is last-stage-at-or-below).
#
# THE STOP GATE (do not skip): after step 12 (rounds 1+2), post-hoc eval round 1's suite
# (object) at temp 1.0, 50 envs. Held -> the masked-self anchor works under OPD, continue.
# Dropped -> the mechanism failed; stop at 18h instead of 36h.
#
# Run it under the watchdog, never bare:
#   SR_PROC_MAX=1000 bash /share/fanruochen-local/dev/scripts/safe_run.sh \
#     /share/fanruochen-local/outputs/opd_seqslot_driver.log \
#     bash examples/embodiment/incremental_sft/opd_seqslot.sh
set -uo pipefail

TAG="${TAG:-seqslot}"
STEPS="${STEPS:-24}"                       # 4 rounds x 6 steps; the round table lives in the config
GPUS="${GPUS:-0,1,2,3}"                    # four; 192/(8*4) = 6 grad-accum, mt4slotA-proven
PORT="${PORT:-58000}"                      # isolated ray head; keep >=1000 from any other job
SAVE_INTERVAL="${SAVE_INTERVAL:-6}"        # = round length, so saves land on round ends
O=/share/fanruochen-local/outputs
STUDENT="${STUDENT:-$O/inc_sft_opd/lwf_long_e1000_merged}"
REPO=/home/fanruochen/CL/RLinf
CFG=libero_seqslot_4gpu
...
for g in ${GPUS//,/ }; do
  ...
  free=$(nvidia-smi -i "$g" --query-gpu=memory.free --format=csv,noheader,nounits) || {
    echo "ABORT: cannot query GPU$g"; exit 1; }
  # The question is "is there room for this job", NOT "has anyone else touched the card".
  # The old test was `used < 5000`, which on an 80 GB card refuses a GPU with 55 GB free
  # just because a tenant holds 25 GB -- and every card on this box is like that, so it
  # capped the run at 2 GPUs while 4 were usable (2026-08-23, with a deadline).
  # NEED_FREE_MIB is what this configuration actually needs resident, with headroom for
  # the checkpoint gather: measured peak on 2 ranks was 97% of 80 GB at save time, and
  # 4-way sharding roughly halves the per-rank share of that.
  NEED_FREE_MIB="${NEED_FREE_MIB:-45000}"
  (( free >= NEED_FREE_MIB )) || {
    echo "ABORT: GPU$g has only ${free} MiB free (${used} MiB held by others), need"
    echo "       >=${NEED_FREE_MIB}. Lower NEED_FREE_MIB only with a micro_batch_size"
    echo "       that actually fits -- an OOM mid-run costs more than waiting."
    exit 1; }
  busy=0
  for _ in 1 2 3; do
  ...
  (( busy < 2 )) || {
    # ALLOW_BUSY_GPU=1 is the ONE way past this check, and it exists so that co-running on a
    # memory-light / compute-heavy tenant job is always a VISIBLE, typed decision that stays in
    # the shell history -- never a quietly lowered threshold. It is defensible only when the card
    # has plenty of FREE MEMORY and we are resuming a co-tenancy we already had (2026-08-22: the
    # R1 run shared GPU1/GPU3 with exactly such a ~2GB/80% job for 7 hours; io_guard killed us on
    # a disk red line, not the tenant, and restarting there displaces nobody).
    if [ "${ALLOW_BUSY_GPU:-0}" = "1" ]; then
      echo "WARN: GPU$g at ${util}% util with ${used} MiB held -- ALLOW_BUSY_GPU=1, co-running."
    else
      echo "ABORT: GPU$g at ${util}% util (memory looked free, compute is not) -- not ours to take"
      echo "       set ALLOW_BUSY_GPU=1 to co-run anyway (only when free MEMORY is ample)."
      exit 1
    fi; }
done
