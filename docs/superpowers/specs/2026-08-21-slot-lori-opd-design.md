...
- 全过程在 **`torch.autocast(enabled=False)`** 里做。autocast 是按 op 拦截的，显式 cast 成 fp32 的张量照样被 `matmul` 降回 bf16：实测在 `autocast(bfloat16)` 内 orth_error 2.5e-2、外面 4.4e-6，差 4 个数量级且不报任何错。学生前向恰好就在 `self.amp_context` 里（`fsdp_actor_worker.py:2130`）。
...
- `slot/orth_err` = ‖ĀĀᵀ − I‖_F。**阈值必须按 bf16 的舍入地板定，不是 1e-6。** 诊断读到的是 cast 回 bf16 之后的 Ā，实测 R=256 下这个地板就是 **0.0189（d_in=1024）/ 0.0094（d_in=4096）**；同一个 Ā 在 fp32 下才是 ~4e-5。判据：**< 0.05 健康；> 0.2 说明正交性真的塌了**（独立探针在正交性塌掉时测到 0.75，与 0.019 的地板可清楚区分）。按 1e-6 卡会在第 1 步把一个正确的 run 误判成实现有 bug
- `slot/cos_st` = cos⟨ΔW_s, ΔW_t⟩_F 全部 6 个 pair。同样受 bf16 地板限制（Ā 的 Gram 非对角块并非精确 0），判据取 **|cos| < 0.05 健康、> 0.2 为塌**，与 orth_err 同源同标定。**不要展开 ΔW**（d_out×d_in 太贵）：用 ⟨ΔW_s,ΔW_t⟩_F = tr(B_sᵀB_t · Ā_tĀ_sᵀ) 只算 R×R 的小矩阵，Ā 的 Gram 前向里已经有了
