...
**scale 折进 Ā（而不是事后乘）**：`delta_weight()` 与 forward 走同一条路线，scale 在 `Ā` 一侧折入。在 d_in 是 2 的幂时（1024→s=0.25、4096→s=0.5）两种放法逐位相同；在 2048 / 11008 这类宽度上折入更准。**注意：这个优势的绝对数值依赖测量设置**（d_out、batch、B 的初始化尺度）——三次独立测量分别得到 3.677e-3 vs 3.996e-3、3.78e-3 vs 4.13e-3、2.41e-3 vs 2.90e-3。三者方向一致、差距都在 15-20%，但绝对值不可当作固定常数引用。测试只钉住 bitwise 事实，不钉绝对值。
